...
def generate_readme(stats):
    ...
    md = [
        ...
        f"",
        f"* **Mihomo (.mrs)** : 二进制编译格式，加载速度快，资源占用极低。双重生成策略，把域名裂变为 `精确匹配` 与 `泛域名匹配` ，解决子域名匹配遗漏问题",
        f"* **Loon (.lsr)** : 纯文本格式，已优化混合负载结构。优先匹配带有 `no-resolve` 属性的规则，减少不必要的 DNS 解析行为，降低 DNS 泄露风险",
        f"",
        f"* **GeoSite_CN**：剔除死链、伪直连（指纹识别）和境外 CDN 域名，同时经过 DNS 验活、CNAME 查杀、IP 物理核查过滤，确保 CN 域名稳定和精准",
        f"* **GeoIP_CN**：剔除 Cloudflare/Google 等境外 IP，仅保留物理位置在中国内陆的 IP，提高纯度与可控性",
        f"",
        f"## 📍 Mihomo 配置指引",
