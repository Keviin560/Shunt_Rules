...
def generate_readme(stats):
    ...
    md = [
        ...
        f"",
        f"## 📖 项目简述",
        f"本项目采用高级**双轨制架构**，每天定时从上游拉取十万级数据，通过剧毒黑洞比对、DNS深度清洗与语法转译，输出完美贴合内核的极纯净分流规则",
        f"",
        f"* Mihomo (.mrs) : 用双重生成策略，把域名裂变为 `精确匹配 (DOMAIN)` 与 `泛域名匹配 (DOMAIN-SUFFIX)` ，彻底解决子域名分流遗漏问题",
        f"* Loon (.lsr) : 前置命中带有 `no-resolve` 属性的 IP 规则，大幅减少不必要的 DNS 解析行为，降低 DNS 泄露与污染风险",
        f"",
        f"* GeoSite_CN：采用全满贯抓取，并通过持久化黑洞库剥离大厂海外业务与广告域名，打造极其纯粹的大陆本土白名单",
