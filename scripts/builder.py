...
def get_yaml_links(raw_base_url):
    try:
        ...
        yaml_files = [f for f in os.listdir('.') if os.path.isfile(f) and f.endswith(('.yaml', '.yml'))]
    except Exception:
        js_files = []
        yaml_files = []
...
def generate_readme(stats):
    ...
    md = [
        ...
        f"",
        f"## 📖 项目简述",
        f"每天定时从上游拉取最新数据，通过 DNS 深度清洗和语法转译，输出贴合内核运行逻辑的纯净分流规则",
        f"",
        ...
        f"",
        f"* **AI_Rules**：聚合了主流的 AI 平台（ChatGPT, Claude, Gemini 等）和 AI 开发者平台",
        f"  <br>OpenAI (ChatGPT/Sora)、Anthropic (Claude)、Google (Gemini/谷歌 AI 家族)、xAI (Grok)、Meta AI、Mistral AI、Cursor、Microsoft/GitHub Copilot、Codeium & Windsurf、Devin、Trae & MarsCode、CodeRabbit、Perplexity、Poe、OpenRouter、ElevenLabs、ComfyUI、OpenArt & Clipdrop、Hugging Face、Groq、Cerebras、Dify、Cloudflare AI、LiveKit、Cici/Coze、Manus等",
        f"* **Game_Proxy**：纯国外游戏平台分流，涵盖了主流的 PC 游戏、主机平台、手游大厂和垂直的游戏社区",
        f"  <br>Steam (Valve)、Epic Games、GOG (CD Projekt)、EA (Electronic Arts/Origin)、Ubisoft (Uplay/Ubisoft Connect)、Rockstar Games (Social Club)、Nintendo (任天堂)、Xbox (Microsoft Gaming)、PlayStation (Sony)、Riot Games (拳头游戏)、Blizzard (战网/暴雪)、PUBG (绝地求生)、osu!、Escape from Tarkov (逃离塔科夫)、HoYoverse (米哈游国际服)、Supercell、Yostar (悠星)、Cygames、Garena、Minecraft (我的世界)、Roblox (罗布乐思)、itch.io 等",
        f"",
        f"## 📍 Mihomo 配置",
        ...
        f"  - RULE-SET,Google_IP,MyProxyGroup,no-resolve    # 按需添加 no-resolve 参数",
        f"```",
        f"</details>",
        f"",
        f"## 🤝 致谢",
